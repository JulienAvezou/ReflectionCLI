...
exports.DEFAULT_HEATMAP_AREAS = exports.SCORE_DIMENSIONS = exports.THINKING_MODE_PROMPTS = void 0;
exports.recommendThinkingMode = recommendThinkingMode;
exports.calculateThinkingScore = calculateThinkingScore;
exports.calculateOutsourcingRisk = calculateOutsourcingRisk;
exports.analyzeHeatmap = analyzeHeatmap;
exports.scoreArchetype = scoreArchetype;
exports.improvePrompt = improvePrompt;
exports.generatePractice = generatePractice;
exports.THINKING_MODE_PROMPTS = {
    Explore: [
...
function recommendThinkingMode(goal) {
    ...
    const reasons = {
        Explore: 'You are still forming the problem model, so discovery questions are more useful than implementation prompts.',
        Challenge: 'You already have a direction, so the highest-value move is to stress-test assumptions before building.',
        Decide: 'You are comparing paths, so the work is to make tradeoffs explicit and choose deliberately.',
        Audit: 'You need confidence in correctness, so verification and edge cases should lead the session.',
        Reflect: 'The work is complete enough to extract learning and identify remaining comprehension debt.',
    };
    return {
        mode,
        reason: reasons[mode],
        prompts: exports.THINKING_MODE_PROMPTS[mode],
    };
}
...
function calculateThinkingScore(answers) {
    const dimensions = exports.SCORE_DIMENSIONS.reduce((result, dimension) => {
    ...
    const ordered = exports.SCORE_DIMENSIONS.map((dimension) => [dimension, dimensions[dimension]]);
    ...
    const actions = {
        understanding: 'Before asking AI for code, write a two-sentence model of the problem and one uncertainty.',
...
function calculateOutsourcingRisk(answers) {
    const riskAreas = [];
    ...
    const risk = riskPoints >= 6 ? 'High' : riskPoints >= 3 ? 'Medium' : 'Low';
    const suggestedNextAction = risk === 'High'
...
function analyzeHeatmap(areas, scale) {
    if (scale !== 5 && scale !== 10) {
...
function scoreArchetype(answers) {
    ...
    answers.forEach((answer) => {
        if (!validArchetypes.includes(answer)) {
            throw new Error('Archetype answers must be one of: AI Autopilot, AI Assistant, AI Challenger, AI Architect');
        }
        scores[answer] += 1;
    });
    ...
    const explanations = {
        'AI Autopilot': 'You may be letting AI carry too much judgment. The next improvement is verification before acceptance.',
        'AI Assistant': 'You use AI mainly for execution support while keeping much of the direction yourself.',
        'AI Challenger': 'You are using AI to test and improve your thinking, which is a strong collaboration pattern.',
        'AI Architect': 'You use AI strategically while preserving ownership of system direction and tradeoffs.',
    };
    const actions = {
        'AI Autopilot': [
...
function generatePractice(log) {
    ...
    return {
        ...
        reflectionQuestion: 'What did I genuinely understand, and what should become comprehension debt?',
